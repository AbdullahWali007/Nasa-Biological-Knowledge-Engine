...
Put index.json and SB_publication_PMC.json in same folder. Set GEMINI_API_KEY env var or edit GEMINI_API_KEY below.
"""
...
class SearchIndex:
    """
    Builds a TF-IDF index over datasets and publications using:
      - dataset id
      - metadata fields
      - column names
      - textual sample rows
    ...
    def _build_index(self):
        ...
        for idx, pub in enumerate(self.publications):
            title = pub.get("Title", "")
            link = pub.get("Link", "")
            # Create searchable text for publication
    ...
    def query(self, q: str, top_k_datasets: int = TOP_K_DATASETS, top_k_publications: int = TOP_K_PUBLICATIONS) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        Return ranked datasets and publications with combined scoring.
        Returns: (datasets, publications)
        """
        q_norm = q.strip()
        ...
        # Separate datasets and publications
        dataset_results = []
        publication_results = []
...
class EngineRunner:
    ...
    def ask(self, question: str, prefer_dataset_urls_first: bool = True) -> Dict[str, Any]:
        # find relevant datasets and publications using improved index
        ...
        # Improved Gemini prompt with NASA-specific search instructions and publications
        prompt = (
            "SYSTEM: You are a NASA Space Biology expert assistant with web search capabilities. "
            "Follow these guidelines precisely:\n\n"
            "1. FIRST analyze the provided DATA CONTEXT thoroughly including datasets and research publications\n"
            ...
            f"DATA CONTEXT:\n{combined_context or '[NO RELEVANT LOCAL DATASETS OR PUBLICATIONS FOUND - PLEASE SEARCH NASA REPOSITORIES]'}\n\n"
            f"USER QUESTION:\n{question}\n\n"
        ...
        # Combine URLs from datasets, publications, and response
        combined_urls = []
        if prefer_dataset_urls_first:
            # Add dataset URLs first
...
def main():
    ...
    if ask_btn and query:
        with st.spinner("🔬 Analyzing question for NASA space biology relevance..."):
            ...
            # Find relevant datasets and publications
            with st.status("Searching NASA space biology datasets and publications...", expanded=True) as status:
                ...
                for c in publication_cand[:3]:  # Show top 3
    ...
    if st.session_state.current_result:
        ...
        with tab5:
            ...
            out_meta = {
                ...
                "used_publications": [p for p in used_publications],
                "structured": structured
            }
